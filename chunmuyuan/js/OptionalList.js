...
define(["base/baseApp", "refresher", "animate", "./base/baseCar", "./base/basePage","./base/discount"], function(app, refresher, animate, car, page, discount) {
	...
	app.addList = function(data) {
		...
		for(var i = 0; i < data.length; i++) {
			...
			var el = $(this.template(item));
			var inputMnieus = el.find(".inputMnieus");
			app.fastclickNumber(inputMnieus,item.id,app.clickMinus);
			var inputPlus = el.find(".inputPlus");
			app.fastclickNumber(inputPlus,item.id,app.clickPlus);
			$("#list").append(el);
		}
	...
	app.init = function() {
		...
		app.getUser(function(data) {
			app.imLinkUrlReal = app.imLinkUrl+ "&nickName=" + data.crmnickname
		});
		initRefresh(); //初始化下拉组件
