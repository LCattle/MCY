(function() {
	...
	window.isInAppEnviroment = function() {
		if(isInSpringWoodsAndroid() || isSpringWoodsIos()) {
			return true;
		}
		return false;
	}
	/**
	 * 判断是否在springwoods的安卓环境
	 */
	window.isInSpringWoodsAndroid = function() {
		var ua = window.navigator.userAgent.toLowerCase();
		if(ua.indexOf("springwoods_android") != -1) {
			return true;
		} else {
			return false;
		}
	...
	/**
	 * 判断是否在springwoods的IOS环境
	 */
	window.isSpringWoodsIos = function() {
		var ua = window.navigator.userAgent.toLowerCase();
		if(ua.indexOf("springwoods_ios") != -1) {
			return true;
		} else {
			return false;
		}
	}
	if(window.isSpringWoodsIos()) {
	 ...
	 * 
	 */
	window.nativeRegistBackFunc = function(callBack) {
		var data = {
			funcName: "registBackFunc",
		};
		if(isSpringWoodsIos()) {
			WebViewJavascriptBridge.registerHandler('onNativeBack', function(data, responseCallBack) {
				app.onNativeBack();
			});
		}
		return callNativeFunc(data);
	 ...
	 * 刷新token
	 */
	window.refreshLoginToken = function(callBack) {
	...
	window.callAndroidFunc = function(data) {
		if(!isInSpringWoodsAndroid()) { //不是在安卓环境直接returnfalse
			return false;
		}
	...
	/**
	 * 设置共享数据
	 * type说明：2本次启动游戏，1永久存储 
	 */
	window.nativeSetShareData = function(key, val, type) {
		var data = {
			funcName: "setShareData",
			param: {
				key: key,
				value: val,
				type: type + ""
	 ...
	 * 获取共享数据
	 */
	window.nativeGetShareData = function(key, type, callBack) {
		var data = {
			funcName: "getShareData",
			param: {
				key: key,
				type: type + ""
			},
		...
		//		//安卓特殊处理
		//		if(window.mobile &&
		//			window.mobile[data.funcName] instanceof Function) {
		//			var paramStr = JSON.stringify(data.param);
		//			var val = window.mobile[data.funcName](paramStr);
		//			callBack(val);
		//			return true;
		//		}
		return callNativeFunc(data);
	...
	/**
	 * 打开原生界面
	 * viewName:"indexPage"首页，
	 * userPage 用户中心
	 * loginPage:登录页
	 * 
	 */
	window.nativeOpenNativeView = function(viewName, extraData) {
		var data = {
			funcName: "openNativeView",
			param: {
				viewName: viewName,
				extraData: extraData
			},
	 ...
	 * 回调一个好友对象
	 */
	window.nativeOpenSelectAFriend = function(callBack) {
		var data = {
			funcName: "openSelectAFriend",
			callBack: callBack
		}
		return callNativeFunc(data);
	}
	
	window.nativeOpenAppendAFriend = function(param) {
		var data = {
			funcName: "localFriendInfo",
			param: param,
			paramType: 1
		}
		return callNativeFunc(data);
	}
	window.nativeAccount = function(param) {
		var data = {
			funcName: "openSignDialog",
			param: param,
			paramType: 1
		}
		return callNativeFunc(data);
