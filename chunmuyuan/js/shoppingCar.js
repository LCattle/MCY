...
define(["base/baseApp", "refresher", "animate", "./base/baseCar","./base/discount"], function(app, refresher, animate, car, discount) {
	...
	app.refreshData = function(){
		app.loading('加载中...',true);
		car.init(function() {
			loadGoods();
		});
        //app.replace("shoppingCar.html");//刷新整个购物车页面
	};
	if(window.WebViewJavascriptBridge){
		WebViewJavascriptBridge
			.registerHandler("refreshData",
				function(){
					app.refreshData();
				});
	}
	 ...
	 * 
	 */
	app.fastclickNumber = function(El,sku,fn){
			var isMove = false;
		...
		if(data.length > 0) {
			...
			if(!app.checkLogin()){//未登录购物车显示状态
				for(var i = 0; i < data.length; i++) {
					...
					var el = $(this.template(data[i]));
					var inputMnieus = el.find(".inputMnieus");
					app.fastclickNumber(inputMnieus,data[i].id,app.clickMinus);
					var inputPlus = el.find(".inputPlus");
					app.fastclickNumber(inputPlus,data[i].id,app.clickPlus);
					$("#list").append(el);
			    }
				$("#bookPrice").text("总计:");
			}else{//登录之后购物车显示状态
	...
	function loadGoods() {
		...
		if(ids.length < 1) {
			app.hideLoading();
			app.emptyList("#list","暂无商品 <span class='app-text-orange' onclick = 'location.href=\"OptionalList.html\"'>去逛逛</span>");
	...
	app.initPage = function() {
		...
		var h = $(window).height() - $("#head")[0].clientHeight - $(".footer-bot")[0].clientHeight - $("#topMess")[0].clientHeight;
