...
define(["base/baseApp", "refresher", "animate", "./base/basePage"], function(app, refresher, animate, page) {
		...
		app.POSTRequest("weixin/mall/sku/skuLists.do", {
			...
			success: function(data) {
				...
				if(data.resultCode === "1") {
					...
					if(!page.hasNextPage){
						$("#list").append('<div class="border-radius15 aui-btn aui-btn-block app-text-green border_green zixuan">自选菜</div>');
						$(".zixuan").on("click",function(){
							location.href="../html/OptionalList.html";
						})
					}
					page.pageIndex++;
	...
	app.gotoConfirmOrder = function(id,e) {
		app.setLocalObjet("selectProduts", [{
				skuid: id,
				num: 1
		}]);
		if(!app.checkLogin()) {
	    	app.toLoginPage(app.netConfig.pageRoot + "/html/confirmOrder.html?type=" + 2);
		}else{
			app.stopBubble(e);
		    location.href = "../html/confirmOrder.html?type="+2;
		}
	}
	...
	app.initPage = function() {
		...
		var h = $(window).height() - $("header").height() - $("#box").height();
	...
	//开始执行
	app.init = function() {
		var high = $(window).height() - $("#box").height() - 1;
		$("#wrapper").css("height",high + "px");
		initRefresh();
		loadData();
	};
	return app;
