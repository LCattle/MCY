...
define(["base/baseApp", "./base/order"], function(app, Order) {
	...
	function loadOrder() {
		Order.getOrder(app.UrlParams.oid, function(order) {
			...
			showBtns(order);
			showLogistics(order);
			$("#logisticsprice").html('+' + app.getPriceStr(order.freightagefee));
			...
			if(order.type == 1 || order.type == 2 || order.type == 7 || order.type == 9) {
			...
			if((order.type == 3 || order.type == 4) && order.payway == 30){
				$("#priceWait").text("待付款");
			}
	...
	function showProduct1(order) {
		if(order.isneworder == 'Y'){//新订单
			...
			$("#farePrice").html(app.getPriceStr(app.goodsVerVal));
	...
	function showProduct10(order) {
		...
		$("#farePrice").html(app.getPriceStr(totalPrice));
		$.each(data, function(i, item) {
			if(item) {
				if(i > 4) {
					item.hide = true;
	...
	app.toPay = function(id) {
		var order = app.order;
		var page = "../html/confirmpay.html";
		var kobe = "../html/confirmpay.html";
		if(isInAppEnviroment()) {
			page = "../payment/confirmpay.html";
			kobe = "../payment/newlyPay.html";
		}
		if(order.type == app.ORDER_TYPE_OPTION ||
			order.type == app.ORDER_TYPE_PACKAGE) {

			app.href(page + "?oid=" + order.id + "&ono=" + order.orderno +
				"&money=" + order.virtualfee + "&type=" + order.type);
		} else {
			app.href(kobe + "?oid=" + order.id + "&ono=" + order.orderno +
