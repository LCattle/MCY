...
define(["base/baseApp", "./base/order"], function(app, Order) {
	...
	app.toOrder = function() {
		location.href = "../html/giftDetails.html?oid=" + app.UrlParams.oid;
	};
	//开始执行
	app.init = function() {
		loadOrder();
	...
	function loadOrder() {
		...
		Order.getOrder(app.UrlParams.oid, function(order) {
			...
			if((order.type == 1 || order.type == 2 || order.type == 3 || order.type == 10) && (order.message != "")){//备注
				$("#remark").show();
			...
			if(order.type == 1 || order.type == 2) {
				...
				$.each(order.items,function(i,item){
					app.carriage += (item.sellprice*1 - item.actualpaied*1)*(item.amount*1)
				})
				$("#vipFare").text('¥' + (app.carriage).toFixed(2));
				...
				app.POSTRequest("weixin/mall/order/obtailValue.do", {//成长值
					data: {
						loginedtoken: Token,
				...
				if(app.UrlParams.deliverymode==30){//自提
                    if(app.UrlParams.modify == 1){
                       ...
                       // showVarval(order.modifyfee);
                        ...
                        showAddress(order);
                        $("#paystatus").append("<h2 class='app-text-orange' style='display:inline'>您已兑换成功!<span></h2>待自提~");
                    ...
                    if(app.UrlParams.modify == 1){
                        ...
                        showAddress(order);
                        $("#paystatus").append("<h2 class='app-text-orange' style='display:inline'>您已兑换成功!<span></h2>待发货~");
                        //showVarval(order);
                        showPreferential(order);
                        //showFare(order.freightagefee);//运费
                        $(".disappear").show();
                ...
                $("#paystatus").append("<h2 class='app-text-orange' style='display:inline'>您已兑换成功!</h2>待发货~ <br/>您的专属顾问将联系您为您送货上门");
                $("#number").html(app.getPriceStr(order.sprices));
                $("#priceLable").text("商品金额：");
                $("#totalbox .app-text-orange").html(app.getPriceStr(order.actualpaied));
                $("#totalbox").show();
