...
define(["base/baseApp",
	...
	function(app, slide, animate, car, cardCar, page) {
		...
		//加载商品信息 加载的是静态js
		function loadGoods(productInfo) {
		...
		function initData(productInfo) {
			...
			initBanner(productInfo.skupicturelist);
		...
		function toConfirm(skuId, num, type) {
			...
			if(app.checkLogin()){
				...
				app.toLoginPage("../html/confirmOrder.html?type=" + type);
			}
		}
				...
				app.POSTRequest("weixin/mall/shoppingcart/merge.do", {
					...
					success: function(data) {
						if(data.resultCode === "1") {
							app.href("../html/OptionalDishes.html?ids=" + ids);
						} else {
							app.tipInfo(data.resultMsg);
						}
					},
