...
define(["base/baseApp", "dialog", "picker", "./base/baseCar", "./base/baseCardCar", "./base/order"], function(app, dialog, Picker, car, cardCar, Order) {
	...
	app.init = function() {
		...
		$("#buyBtn").bind("click", function() { //绑定提交订单按钮
			if(app.orderType == ""){
			    app.tipInfo("请添加地址");
			} else if(app.getLocalObject("yx_selectAddress").cityid != 1607){
                app.tipInfo("仅限深圳地区用户参与活动");
			}else{
				var paramdata = {
                    loginedtoken:lt,
                    addressId:app.getLocalObject("yx_selectAddress").id,
                    delivertime:app.DeliveryDate,
                    goods:skuid+":1"
                };
                app.POSTRequest("weixin/mall/orderhd/deliveryGoodsSubmitHdzsOrder.do", {
                    data:paramdata,
                    success: function(data) {
                        if(data.resultCode == 1) {
							var oid = data.resultObj[0].id;
                            localStorage["ty_selectAddress"] = "";
                            app.replace("../html/yx_paysuccess.html?oid="+oid);
                        } else {
	...
	function initAddressInfo() {
		...
		$("#addressAdd").bind("click", function() {
			...
			location.href = "addressEdit_yx.html";
		});
		$("#addressInfo").bind("click", function() {
            localStorage.selectAdressBackUrl = location.href;
            location.href = "addressSelect_yx.html";
		});
	}
	/**加载地址信息*/
	function loadAddress() {
		//获取本地存储的地址
		var data = app.getLocalObject("yx_selectAddress");
		...
		app.POSTRequest_m("/member/address/getDefaultAddress.do", {
			success: function(data) {
				if(data.resultCode == 1) {
					...
					app.setLocalObjet("yx_selectAddress", address);
					initAddressInfo();
				} else {
					app.tipInfo(data.resultMsg);
				}
